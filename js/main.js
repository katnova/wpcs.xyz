...
async function loadScript(url, context) {
  await loadExternalScript(url).then(() => {
    ...
    if (debug)
      console.debug(
        log_level_debug + "--------START MODULE CONSOLE OUTPUT--------"
      );
    try {
      run(context);
    } catch (e) {
...
function loadExternalScript(src) {
  if (debug)
    console.debug(
      log_level_debug + "Loading temporary external script from: " + src
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    if (debug)
      console.debug(log_level_debug + "Added script tag to body: ", scriptTag);
  });
}
...
function removeTempScripts() {
  for (let i = 0; i < loadedScripts.length; i++)
    try {
      ...
      if (debug)
        console.debug(
          log_level_debug + "Removed script with ID: " + loadedScripts[i]
        );
    } catch (e) {
      if (debug)
        console.debug(
          log_level_debug + "Failed to unload script at position " + i
        );
    }
  loadedScripts = [];
}
  ...
  if (!debug)
    for (let i = 0; i < jsFilePaths.length; i++) {
      if (!authorized_scripts.includes(jsFilePaths[i])) {
        context.echo(
          log_level_warn +
            red("Unauthorized loaded script found. src: " + jsFilePaths[i])
        );
        if (debug)
          console.debug(
            log_level_warn +
              "Unauthorized loaded script found: " +
              jsFilePaths[i]
          );
        res = false;
      } else {
        if (debug)
          console.debug(
            log_level_debug + "Authorized script found: " + jsFilePaths[i]
          );
      }
    }
  return res;
...
function resolve_module(context) {
  if (debug)
    console.debug(
      log_level_debug + "---------END MODULE CONSOLE OUTPUT---------"
    );
  if (debug) console.debug(log_level_debug + "Cleaning up post module run...");
  if (module_loading_messages)
    context.echo(log_marker + "Cleaning up module...");
  removeTempScripts();
  context.set_prompt(term_prompt);
  if (debug)
    console.debug(log_level_debug + "Set terminal prompt to: ", term_prompt);
  if (module_loading_messages)
    context.echo(log_marker + "Verifying clean-up....");
...
function verifyLoadedScripts(context) {
  ...
  if (checkLoadedScripts(context)) {
    ...
    context.echo(
      log_marker +
        red(
          "Unknown scripts found after module unload, if you don't recognise them, please reload the page."
        )
    );
  }
}
